...
class FilmWatched{
	...
	{
		$contents = array();
		$q = $db->query('SELECT * FROM `films_watched` ORDER BY `id` DESC;');
		while ($datas = $q->fetch(PDO::FETCH_ASSOC))
		{
			$datas['id'] = (int) $datas['id'];
			$contents[] = new FilmWatched($datas);
		}
		return $contents;
	}

	public static function findOne($id_films,$id_users,$db)//return a FilmWatched object
	{

		$data = $db->query("SELECT * FROM `films_watched` WHERE `id_films` = $id_films AND `id_users` = $id_users;")->fetch(PDO::FETCH_ASSOC);
		$data['id'] = (int) $data['id'];
		$data['id_films'] = (int) $data['id_films'];
		$data['id_users'] = (int) $data['id_users'];
		try{return $data = new FilmWatched($data);}
		catch(Exception $e){
			return new Array();
		}
	}	
	...
	{
		extract($data);
		return $db->exec("INSERT INTO `films_watched` (`id`,`id_users`,`id_films`) VALUES ('','$id_users','$id_films');");
	}

	public function delete($film_watched,$db)
	{
		$id = $film_watched->getId();
		$query = "DELETE FROM `films_watched` WHERE `id` = $id";
